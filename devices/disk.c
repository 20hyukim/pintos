...
void
disk_init (void) { // 디스크와 채널 초기화를 통해, 디스크 장치를 시스템에서 사용할 수 있도록 설정. 인터럽트를 활용해 효율적인 I/O 처리를 지원.  
	size_t chan_no;

	for (chan_no = 0; chan_no < CHANNEL_CNT; chan_no++) { // 디스크의 모든 채널에 대해 순회하면서 초기화
		struct channel *c = &channels[chan_no];
		int dev_no;

		/* Initialize channel. */
		snprintf (c->name, sizeof c->name, "hd%zu", chan_no); // 채널 이름을 hd0, hd1 ... 이런 형식으로 설정.
		switch (chan_no) {
			case 0:
				c->reg_base = 0x1f0; // 0번 채널의 기본 I/O 주소
				c->irq = 14 + 0x20; // 1번 채널의 
				break;
			case 1:
				c->reg_base = 0x170;
				c->irq = 15 + 0x20;
...
1:1 - swap
*/
/* 디스크 번호 DEV_NO에 해당하는 디스크를 반환.
 * DEV_NO는 master(0) 또는 slave(1)를 나타내며, 
 * CHAN_NO는 채널 번호를 나타냄.
 * 
 * Pintos의 경우 다음과 같이 디스크 사용.
 *   0:0 - 부트 로더, 명령줄 인자, 운영 체제 커널
 *   0:1 - 파일 시스템
 *   1:0 - 스크래치 공간
 *   1:1 - 스왑 공간
 */

/* 함수의 목적: 지정된 디스크에 해당하는 디스크를 반환함.*/
struct disk *
